...
describe('capability presets follow the Development Plan', () => {
  it('exposes users, customers and service catalogs in HTTP mode even with prototype flags', () => {
    ...
    expect(
      Object.entries(capabilities)
        .filter(([key]) => key !== 'users' && key !== 'customers' && key !== 'catalogs')
  ...
  it('treats an unset mock flag as HTTP Access/Users plus customers and services, not the in-memory prototype', () => {
    const capabilities = resolveCapabilities({
      VITE_CAPABILITIES_PRESET: 'prototype',
      VITE_ENABLE_DEMO_CONTROLS: 'true',
      DEV: true,
